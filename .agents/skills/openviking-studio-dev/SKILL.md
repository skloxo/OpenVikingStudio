...
# OpenViking Studio 专项开发与迭代技能 (OpenViking Studio Dev SOP)

本技能为 OpenViking Studio (`OpenVikingStudio`) 的专用工程开发 SOP。当用户提到 "开发 VK", "重构 Studio", "新增 Studio 页面/卡片", "更新工单" 或处于 OpenVikingStudio 工作区时自动激活。
...
3. **历史交付档案** ➔ [DELIVERY_ARCHIVE.md](file:///home/skloxo/aho/openclaw/project/OpenVikingStudio/DELIVERY_ARCHIVE.md)
   - 归档所有已验收通过的版本履历 (`v1.1.0` ~ `v1.1.23b`+)。
...
2. **物理对齐与无胶水脚本铁律**：
   - 卡片底部底部汇总行统一使用 `mt-auto` 实现 100% 物理对齐。
...
2. **禁止虚假 Mock**：数据缺失时必须优雅显示 `--`，禁止硬编码伪造假数据数字。

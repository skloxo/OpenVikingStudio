...
#### 📌 Lesson 2026-08-01 #22：切除恒定伪指标与共享技能资产库动态量化守则
- **CONTEXT**：以前 Card 2 放置“规范标准上架率 (100%)”，由于 Harness 会全自动静默补全上架，导致该指标恒为 100%，无任何信息增量，属于典型的假指标/噪音。
- **REFLECTION**：遵循奥卡姆剃刀（Occam's Razor），切除无信息增量的伪指标。卡片应当呈现具备真实物理增量的技能规模大字。
- **LESSON**：替换为 `共享技能资产库` 动态指标：
  1. **主大字**：呈现物理真实的 **`160 项`** `标准技能` 动态数量；
  2. **自解释说明**：`已全量装载至 OpenViking 大脑`，直观呈现共享技能资产的规模与健康度。

#### 📌 Lesson 2026-08-01 #23：VK 技能统一收敛率与踩坑演进飞轮指标守则
- **CONTEXT**：以前展示“多 Agent 共享覆盖 (4个Agent已接入)”，属于数手指头都能算清的低维假覆盖率；未反映 Agent 调用技能到底是走私有渠道还是走 VK 技能中心。
- **REFLECTION**：技能中心的最顶层核心价值在于**“能力统一收敛与踩坑演进”**——唯有 Agent 统一走 VK 技能中心，踩坑经验才能反馈给中心演进，让全盘智能体受惠。
- **LESSON**：重构 Card 3 为 `VK 技能统一收敛率`：
...
#### 📌 Lesson 2026-08-01 #25：技能运行成功率全生命周期闭环指标守则
- **CONTEXT**：以前 Card 2 展示“175 项标准技能”，属于下方技能列表眼看即知的低维静态数字，缺乏对“技能跑得怎么样、运行情况如何”的核心监测。
- **REFLECTION**：技能全生命周期的第 2 阶段是“物理执行闭环”。必须监测技能被 Agent 唤醒后的运行成功率与异常率，完成“触发➔执行➔收敛➔进化”4 阶矩阵闭环。
- **LESSON**：重构 Card 2 为 `技能运行成功率`：
  1. **标题**：`技能运行成功率` (Badge: `闭环质量`)；
...
#### 📌 Lesson 2026-08-01 #30：信息层级守则 — 全局属性放全局，局部 Badge 说局部
- **CONTEXT**：添加 24H 时间窗口后，将 `24H 感应 / 24H 闭环 / 24H 收敛` 重复标在每张 KPI 卡片的 Badge 上，既冗余又显蠢。
- **REFLECTION**：时间范围是 4 张卡片共同的全局属性，已在区块标题行统一展示 `🕒 统计范围: 最近 24 小时 (24H Rolling)`，无需在每张卡片上重复。卡片 Badge 应该描述指标本身的**语义维度**，而非全局维度。
- **LESSON**：信息层级 DRY 原则：
  1. **全局属性放全局**：时间窗口、统计口径等共同属性，统一在区块标题或区块 header 行标注一次；

...
### 核心纲领十：Fork 立足之本 —— 前置客观指标锚定与全生命周期正向改进闭环 (Observable Metrics Anchor & Verified Improvement SSOT) ⭐⭐⭐⭐⭐

> **Fork 存在的最高使命与第一性原理**：  
> 官方仓库缺乏贯彻执行力且内部机制黑盒隐匿，这是我们创建并维护此 Fork 分支的核心根由。  
> **严禁盲目迭代**！所有的迭代优化内容必须要有明确目的、有明确改进，不能做无目的的代码堆叠。“如果改动完成后人类在前端界面上看不出任何客观数据指标的改善，人类就根本无法得知这次迭代到底是正向的优化还是反向的退化！”

1. **【开工前客观指标锚定契约 (Pre-Iteration Metric Anchor)】**：
   - 任何功能开发、性能调优或缺陷重构在开工前，**必须在任务卡片 (`REFACTORING_PLAN.md`) 与规格书中显式声明本次迭代衡量的「前端客观数据指标」(Frontend Metric Anchor)**；
   - **两大合法指标形式（二选一，禁止留空）**：
     - **(a) 沿用既有功能指标 (Existing UI Metric Anchor)**：若系统已具备相关衡量指标（如“贯彻准确率”、“门禁拦截率”、“检索召回率 P@5”、“Token SNR”、“端到端延迟”），而本次优化旨在提升它，则开工前必须明确声明以该既有指标为验证标尺；
     - **(b) 配套新增界面数据指标 (New UI Metric Anchor)**：若为全新领域或此前缺乏观测手段的模块，则本次迭代**必须同步配套开发前端界面上的客观数据指标**（如高密指标瓦片、状态图谱、统计卡片或实时交互试验台），让改动效果在 UI 上 100% 直观可衡量，绝不交付纯后台黑盒代码。
2. **【交付时闭环对比核验 (Delivery Gain Verification)】**：
   - 交付汇报时，必须在验收清单中**显式呈现所锚定指标的改进成效**（说明原指标基线值 vs 优化后达成值，或指出 UI 上哪个卡片直观反映了本次改动的收益）；
   - 证明本次改动为明确的**正向收益 (Positive Gain)**，彻底消灭“改了代码但效果未知”的伪优化。

### 核心纲领十一：任务卡片开工前四阶熔炼法 (Pre-Iteration Four-Step Doctrine SSOT) ⭐⭐⭐⭐⭐

> **绝对物理公理**：严禁拿到任务卡片就无脑直接写代码！每迭代一个任务卡片之前，必须按部就班严格执行以下四步熔炼，四步齐备且经推演无懈可击后，方可动手编码实施：

1. **第一阶 · 梳理原始方案 (Baseline Scheme Audit)**：
   - 深入回溯之前在 `REFACTORING_PLAN.md` 制定该卡片时写的原始方案、背景、痛点定义与初衷，全面梳理原始逻辑脉络，绝不遗漏核心设计细节；
2. **第二阶 · CPA 与顶级工程模型完善 (CPA & Multi-Model Intelligence Enrichment)**：
   - 运用 CPA 导师/工兵模型（Claude 5.0 / GPT 5.6 顶尖智囊参谋 + Qwen 3.8 / MiMo 快速工兵），并吸收业界顶级工程模型与学术前沿（微软、阿里、腾讯、DeepSeek 等），对原始方案进行针对性补全与架构加固，补齐所有盲区；
3. **第三阶 · 哲学武器深层拷问 (First Principles & Philosophical Cross-Examination)**：
   - **第一性原理**：回归物理本质与因果律，直击根本原因，不被表象误导；
   - **奥卡姆剃刀**：如无必要勿增实体，切除一切过度工程、冗余定时器与复杂包装；
   - **全生命周期信达雅**：架构高内聚低耦合、类型完备零遮盖；命名见名知意；
   - **鲁棒性与 Fail-Fast**：全链路考虑超时、并发竞争、异常脏数据与优雅降级；单文件严格恪守 100~300 行黄金甜点区；
4. **第四阶 · 红蓝对抗逆向审讯与卡片重构 (Red-Blue Adversarial Grilling & Card Re-crystallization)**：
   - **红方（极端攻击者）**：极限挑刺，假设系统上线后死锁在哪里？性能瓶颈在哪里？并发/断网/大文件撑爆时如何崩溃？是否存在伪需求？
   - **蓝方（防御者）**：针对红方攻击，设计物理级防御门禁与自愈机制；
   - **卡片重塑**：重新梳理完善方案，在 `REFACTORING_PLAN.md` 重构并更新该 Task Card 的最终规格、物理边界、前端客观数据指标与验收门禁。
...
> **核心第一性原理**：没有测试用例、执行回显与客观数据指标的交付是不可信的“黑盒交付”。代码变更、测试用例明细、物理执行数据、前端客观指标与 Git 留痕必须五位一体，作为标准交付契约自动呈现。

每完成一个迭代版本，必须完成以下**六步**，并在每次向用户汇报交付时**100% 自动完整呈现**：
...
4. **🧪 交付必附「自动化测试用例清单与物理执行回显」(Automated Test Manifest & Gates)**：
...
5. **👁️ 交付必附「人工肉眼可视化验收用例」(Human Visual Acceptance Checklist) ⭐⭐⭐⭐⭐**：
   - **零代码门槛人眼契约**：充分尊重人类用户不看代码、不敲终端命令的实际需求，每次交付必须自动生成一份**普通人仅靠浏览器肉眼即可 30 秒验证**的人工用例；
   - **(a) 直达页面链接**：提供可直接点击的浏览器完整 URL（如 `http://127.0.0.1:1933/studio` 或具体子模块路径）；
   - **(b) 鼠标操作步骤**：清晰说明人类点哪个菜单、滚到哪个位置、点击哪个卡片；
   - **(c) 肉眼看点与预期效果**：卡片名称、关键指标标签、颜色规范（中性灰色底色、良好状态为湛蓝/冰青 `cyan-500`）、排版紧凑整齐、字号 $\ge 11\text{px}$；
   - **(d) 负向红线排查**：🚫 绝对严禁看到绿色 (NO GREEN EVER)；🚫 绝对严禁看到 `--`、`NaN`、白屏、错位截断或假数据。
6. **📈 交付必附「前端客观数据指标与正向改进核验」(Objective Frontend Metric Verification & Positive Gain Gate) ⭐⭐⭐⭐⭐**：
   - **指标核验闭环**：对照开工前锚定的前端指标（无论是沿用既有指标还是新增配套指标），汇报真实数值与改善幅度；
   - **正向改进证据**：明确指出“人类在界面哪个位置、哪个卡片能看到该指标？指标相比之前是否有明确正向提升？”，用客观数据证实这是一次正向优化，彻底根除盲目迭代！

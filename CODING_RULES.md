...
### 1.3 页面定位分工铁律 (Page Positioning Rules) ⚠️

每次为新卡片选择放置页面时，必须优先对照以下分工：

| 页面 | 定位 | 内容原则 | 刷新频率 |
| :--- | :--- | :--- | :--- |
| **首页 `/home`** | 使用者视角，30 秒扫一眼 | 高层次业务指标 + 高层次健康摘要。答"我今天用了什么？系统大致正常吗？" | 30~60s |
| **监控页 `/monitoring`** | 运维视角，深度排查 | 底层系统组件状态、详细报错、操作指标。答"哪里出问题了？延迟多少？队列多长？" | 10s |
| **关系图谱 `/graph`** | 知识可视化，独立视角 | Viking 资源节点与 Session 上下文的拓扑关系图，不属于业务也不属于运维 | 手动刷新 |
| **资源页 `/resources`** | 文件与版本管理 | 资源文件浏览、预览、历史版本 Timeline | 按需 |
| **用户页 `/users`** | 权限与账户管理 | 命名空间创建、删除、管理员分配、API Key | 按需 |

**判断口诀**:
- 这个卡片，普通用户每天都要看吗？→ **首页**
- 这个卡片，只有出问题时才需要看吗？→ **监控页**
- 这个卡片，是资源/账号/图谱的专属功能吗？→ **对应功能页**
...
**版本号递增规则**:
- **Minor (Y) 版本号绝对锁定在 `1.1`**：严禁自行升级为 `1.2.0`。所有任务必须在 `v1.1.x` 序列平铺递增（如 `v1.1.3` → `v1.1.4` → `v1.1.10` → `v1.1.12`）。
- 只有用户明确发出“向上进一位”或升级大版本指令时，才可以改动 Y 位。
- 每个版本卡片终验通过后，只需更新 `package.json` 中的 `version` 字段，`Sidebar.tsx` 和 `app-shell.tsx` 通过 `__APP_VERSION__` 全局常量**自动同步**。

**任务卡片命名规范**:
- 每张任务卡片以目标版本号命名，格式：`v1.1.x — 功能描述`
- 保证版本号与功能 1:1 严格绑定对应
...
1. **所有页面数据 100% 由 1933 真实 API 后端驱动，一行都不允许写死。**
2. **严禁以任何形式硬编码 mock 数据、演示数字、假数据在组件中**，包括但不限于：
   - 写死的 `completed: 989`、`total: 413` 之类的具体数字
   - `DEFAULT_*` 常量中包含非零的业务数字
   - 注释掉真实 API 调用、改用假数据
3. **如果 API 接口暂时不确定**，组件初始渲染必须显示 `0` 或 `--`，而不是任何"看起来真实"的假数字。
4. **如果 API 接口根本不存在**，必须停止开发，向用户报告后再决定方向，不得自造数据填充。
5. **唯一允许的"默认值"**：数字类型的 `0`，字符串类型的 `''` 或 `'--'`，Boolean 的 `false`。

违反此规则 = 用假数据糊弄用户，属于严重失职。


## 六、 国际化规范 (i18n Rules) ⚠️ 强制随手同步铁律

> **禁止单独开设 i18n 迭代！开发任何新页面/新卡片/新组件时，必须顺手把 `zh-CN.ts` 与 `en.ts` 的翻译 Key 补齐。**

1. **零中文字符串硬编码**：JSX/TSX 中所有展示给用户的文案（标题、标签、按钮、提示、状态文本），必须通过 `useTranslation()` 的 `t('key')` 形式呈现。
2. **双语同步原则**：在 `src/i18n/locales/zh-CN.ts` 增加新词条时，**必须同时**在 `src/i18n/locales/en.ts` 的相同路径下增加对应的英文词条。
3. **按命名空间归类**：
   - 监控页组件写在 `monitoring` 命名空间下
   - 首页组件写在 `home` 命名空间下
   - 通用组件写在 `common` 命名空间下

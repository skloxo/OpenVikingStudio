...
**版本号 z 递增规则**:
- 每个 Task 终验通过后，只需更新 `package.json` 中的 `version` 字段（z 位 +1）
- `Sidebar.tsx` 和 `app-shell.tsx` 通过 `__APP_VERSION__` 全局常量**自动同步**，无需手动修改
- 版本号在终验通过后、git push 前更新

**任务卡片命名规范**（已更新）:
- 每张任务卡片以目标版本号命名，格式：`v1.1.x — 功能描述`
- 例如：`v1.1.3 — VikingDbCard 向量数据库状态卡片`
- 保证版本号与功能一一对应，便于追溯

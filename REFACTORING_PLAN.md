...
### 📌 P0: [ ] [TASK-HARNESS-WHEELS-01] 开源轮子 (LLMLingua-2 & DSPy) 与 Qwen3 专精模型物理融合 + 探针监控

**模块**：Harness Engine (自演进引擎)  
**工单 ID**：`TASK-HARNESS-WHEELS-01`  
**优先级**：P0 (高优先级)  
**来源**：用户指令 ("结合 Qwen3 专精模型，配合 LLMLingua-2 轮子，解耦配置并在前端接入运行状态监控")  
**预计规模**：S (1 个迭代)

#### 1. 目标与描述
1. 后端集成 `LLMLingua-2` 抽稀切除算法与 `Qwen3-Coder-30B-A3B` (MoE 混合专家，3B 激活/3.8GB 显存) 专精模型重构支持，建立 GPU/CPU 硬件自适应引擎。
2. 后端配置文件开放 `[harness.slm]` 解耦配置接口。
3. Web 监控大盘增加 **`⚡ SLM 提炼模型运行状态探针`** 卡片（实时监控硬件设备 CUDA/CPU、延迟、显存占用与健康状态）。

#### 2. 量化验收标准
- [ ] 后端支持 `LLMLingua-2` 与 `Qwen3-Coder` 提炼调度，配置文件支持 `[harness.slm]` 配置。
- [ ] 前端监控页面显性化露显 SLM 模型运行状态探针（显存占用、CUDA 状态、毫秒级延迟）。

---

### 📌 P0: [ ] [TASK-HARNESS-01] 全无人值守自动化技能提炼与测试门禁闭环引擎

**模块**：Harness Engine (自演进引擎)  
**工单 ID**：`TASK-HARNESS-01`  
**优先级**：P0  
**来源**：用户指导 ("让人类看是不合理的，哪有时间天天看，必须 100% 自动化跑起来")  
**预计规模**：S (1 个迭代)

#### 1. 目标与描述
1. 结合 `LLMLingua-2` 与 `DSPy` 自动探测 166 个闲置技能中重叠度 >75% 的离散片段。
2. 搭建 **AST 语法结构校验门禁 + 触发用例测试门禁**：提炼后自动跑测试用例，准确率 100% 自动覆盖落盘，有降级或语法破坏自动静默回滚。
3. 彻底解除人工审核看 Diff 的伪自动化环节，做到 100% 后台 Daemon 无人值守静默优化。

#### 2. 量化验收标准
- [ ] 具备在线意图碰撞模拟与 100% 自动测试门禁。
- [ ] 验证通过自动覆盖，验证失败自动回滚，并在控制台记录日志战报。

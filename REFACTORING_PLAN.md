...
|:---|:---|:---|:---|:---:|
| **`v1.5.76`** | **Card-Cache-Tier2-LRU-FastHit** | **亚毫秒级 LRU 本地二级缓存引擎、防缓存击穿协议、10k 并发压测与座舱监控总盘 (SSOT)** | 1. **Tier-2 高并发 LRU 缓存引擎 (`openviking/service/cache_tier2_*`)**: 落实 `BLUEPRINT.md` 课题四与高并发性能加速规范，实现基于 `collections.OrderedDict` 与 `threading.RLock` 的线程安全亚毫秒级 LRU 缓存；引入 TTL 过期策略与非阻塞 (`wait=False`) 防缓存击穿/雪崩协议；提供毫秒级压测生成器 (`run_benchmark(iterations=10000)`)；<br>2. **RESTful API 全链路路由 (`openviking/server/routers/cache_tier2.py`, 42行)**: 暴露 `/api/v1/cache/stats`, `/clear`, `/benchmark` 并在 `app.py` 规范挂载；<br>3. **座舱级前端高密交互大盘 (`src/routes/monitoring/-components/tier2-cache-card.tsx`, 198行)**: 严格遵循 NO GREEN EVER 🚫、字号 $\ge 12\text{px}$、单文件黄金甜点区，无缝挂载在 `/studio/home` 核心监控面板；呈现 4 大核心指标瓦片（总查询数、缓存命中率、缓存容量、平均延迟）、进度进度条与一键 10k 并发压测操作台；<br>4. **全套自动化门禁验证**: Card 19 专属单测 6/6 全绿 (0.15s)，版本门禁单测 3/3 全绿，安全扫描 4,480 文件零泄密，Vite 构建 20.61s 零报错，产物烘焙并验证版本 `1.5.76`；运行时服务探测对齐 `1.5.76`；实机浏览器推演验证 100% 成功 (10,000 请求实测 100.0% 命中率，平均延迟 0.0007ms，零绿色)。<br>**Commit Hash**：`a1982548b` | **修改文件**：`openviking/service/cache_tier2_types.py`, `openviking/service/cache_tier2_engine.py`, `openviking/server/routers/cache_tier2.py`, `openviking/server/routers/__init__.py`, `openviking/server/app.py`, `src/routes/monitoring/-components/tier2-cache-card.tsx`, `src/routes/home/route.tsx`, `tests/unit/test_cache_tier2_engine.py`, `package.json`, `openviking/_version.py`, `REFACTORING_PLAN.md` | [x] 已验收通过 ✅ |
| **`v1.5.75`** | **Card-DSPy-MIPO-Prompt-Compiler** | **Stanford DSPy (MIPO) 强类型提示词编译、Bootstrap Few-Shot 示例自优化、Zero-Hallucination 契约门禁与 Studio 演练试验台 (SSOT)** | 1. **DSPy MIPO 编译引擎 (`openviking/service/dspy_compiler_*`)**: 落实 `BLUEPRINT.md` 课题五轮子 #5 (Stanford DSPy MIPO Compiler)，实现输入输出强类型 Schema 规约提取、Bootstrap 少量示例黄金样本按质量与验证状态自优化排序、Strict JSON 强类型输出格式固化、不可变边界与零幻觉护栏注入；<br>2. **RESTful API 全链路路由 (`openviking/server/routers/dspy_compiler.py`, 58行)**: 暴露 `/api/v1/dspy/compile`, `/verify`, `/stats`, `/reset-stats` 并在 `app.py` 规范挂载；<br>3. **客户端便捷扩展 (`src/lib/ov-client/`)**: 为 `ovClient` 补充原生 `get/post/put/delete` 辅助方法，根除组件额外适配层；<br>4. **座舱级前端交互套件 (`src/routes/retrieval/-components/dspy-compiler-cockpit.tsx`, 265行)**: 严格遵循 NO GREEN EVER 🚫、字号 $\ge 12\text{px}$、单文件黄金甜点区，挂载在 `/studio/retrieval` 第 11 级专属 Tab，呈现 4 大核心指标瓦片（原始 Token、编译后 Token、契约状态 PASS 冰青 `cyan-500`、耗时 0.87ms）、三大工业级预设、实时双栏对比与 Few-Shot 样本抽屉；<br>5. **全套自动化门禁验证**: Card 18 专属单测 5/5 全绿 (2.93s)，版本门禁单测 3/3 全绿，安全扫描 4,473 文件零泄密，Vite 构建 22.65s 零报错，产物烘焙并验证版本 `1.5.75`；运行时服务探测对齐 `1.5.75`；实机浏览器推演验证 100% 成功 (0.87ms 编译, PASS 零幻觉)。<br>**Commit Hash**：`827eb1bcc` | **修改文件**：`openviking/service/dspy_compiler_types.py`, `openviking/service/dspy_compiler_engine.py`, `openviking/server/routers/dspy_compiler.py`, `openviking/server/routers/__init__.py`, `openviking/server/app.py`, `src/lib/ov-client/client.ts`, `src/lib/ov-client/types.ts`, `src/routes/retrieval/-components/dspy-compiler-cockpit.tsx`, `src/routes/retrieval/-constants/dspy-presets.ts`, `src/routes/retrieval/-types/dspy-compiler.ts`, `src/routes/retrieval/route.tsx`, `tests/unit/test_dspy_compiler_pipeline.py`, `package.json`, `openviking/_version.py`, `REFACTORING_PLAN.md` | [x] 已验收通过 ✅ |
| **`v1.5.74`** | **Card-Context-Router-Pipeline** | **异构多引擎上下文路由网关、自适应语义分段与统一重组管线 (SSOT)** | 1. **异构多引擎自适应上下文路由网关 (`openviking/service/context_router_*`)**: 落实 `BLUEPRINT.md` Topic 5 Section 4.2，调度 Native Caching、LLMLingua-2、PointFive TokenShift、SkillZip 四大引擎；自适应语义分段 5 类异构提示词片段；严格保持先后顺序无损重组；<br>2. **RESTful API 全链路端点 (`openviking/server/routers/context_router.py`, 67行)**: 暴露 `/api/v1/context-router/route`, `/segment`, `/stats`, `/reset-stats` 并规范挂载；<br>3. **座舱级前端交互套件 (`src/routes/retrieval/-components/context-router-cockpit.tsx`, 373行)**: 严格遵循 NO GREEN EVER 🚫、字号 $\ge 12\text{px}$、代码规范切分，呈现 4 大核心指标瓦片、三大异构典型预设、实时双栏对比与分段流水总表；<br>4. **全套自动化门禁验证**: Card 17 专属单测 7/7 全绿 (3.41s)，版本门禁单测 3/3 全绿，安全扫描 4,473 文件零泄密，Vite 构建 21.69s 零报错，产物烘焙并验证版本 `1.5.74`；运行时服务重启对齐 `1.5.74`；实机浏览器走查验证通过。<br>**Commit Hash**：`db0a6f256` | **修改文件**：`openviking/service/context_router_*`, `openviking/server/routers/context_router.py`, `src/routes/retrieval/-components/context-router-cockpit.tsx`, `tests/unit/test_context_router_pipeline.py`, `package.json`, `openviking/_version.py`, `REFACTORING_PLAN.md` | [x] 已验收通过 ✅ |
...
| **`v1.5.59`** | **Card-Mount-FUSE-ReadOnly-MVP** | **OpenViking 虚拟知识库 FUSE 只读挂载 MVP：打通 POSIX 5 大契约 (getattr/readdir/open/read/statfs)、修复幽灵目录缺陷、统一 EROFS 拦截与纯 Python 降级自愈 (SSOT)** | 1. **打通 POSIX 只读 5 大核心系统调用契约 (`viking_fuse.py`)**: 全面重构 `OpenVikingFUSE`，打通 `getattr`、`readdir`、`open`、`read`、`statfs`，支持将 `viking://` 虚拟知识库无缝挂载为本地原生目录（VS Code、Obsidian 等工具可原生只读浏览）；<br>2. **彻底修复幽灵目录缺陷与肃清 9 处未闭环伪异常**: 修复 `getattr` 在目标不存在时返回虚拟目录的严重 Bug，严密抛出 `FuseOSError(errno.ENOENT)`；全量消灭 `mount.py`、`fuse_proxy.py`、`fuse_simple.py`、`fuse_simple_debug.py`、`fuse_finder.py` 中 9 处裸抛 `NotImplementedError`，写操作严格遵守只读契约返回标准 POSIX `errno.EROFS`；<br>3. **无缝环境降级与纯 Python 抽象层**: 针对未安装 `libfuse`/`fusepy` 的环境提供轻量安全基类，保证在 CI 或远程测试环境下 100% 可导入且逻辑单测全绿；<br>4. **完备单元测试套件全绿通过 (`test_viking_fuse_readonly.py`)**: 编写 14 项 POSIX 核心契约与只读写入拦截单测，14/14 项全部通过 (0.25s)；<br>5. **门禁与自动化发布闭环**: Vite 生产构建 21.14s 零报错通过，安全扫描 4,425 文件零泄露，版本号同步自增至 `1.5.59`。<br>**Commit Hash**：`d936b689c` | **修改文件**：`bot/vikingbot/openviking_mount/viking_fuse.py`, `bot/vikingbot/openviking_mount/mount.py`, `bot/vikingbot/openviking_mount/fuse_proxy.py`, `bot/vikingbot/openviking_mount/fuse_simple.py`, `bot/vikingbot/openviking_mount/fuse_simple_debug.py`, `bot/vikingbot/openviking_mount/fuse_finder.py`, `tests/unit/test_viking_fuse_readonly.py`, `package.json`, `openviking/_version.py`, `REFACTORING_PLAN.md` | [x] 已验收通过 ✅ |
| **`v1.5.58`** | **Card-Concurrency-SQLite-Lock-And-EventLoop-Hotfix** | **消灭高并发死锁与线程阻塞：全系统 5 处 SQLite 30s 锁超时治理、WAL 模式与 busy_timeout 加固、20 线程并发压测验证 (SSOT)** | 1. **全链路 SQLite 30s 锁超时加固**: 针对排查出的 5 处裸 `sqlite3.connect`（`telemetry_store.py` 2处、`active_notes_history.py`、`usage_audit/sqlite_store.py`、`cursor_store.py`、`oauth/storage.py`），全量注入 `timeout=30.0` 并配置 `PRAGMA journal_mode=WAL` 与 `PRAGMA busy_timeout=30000`，彻底消灭并发读写下的 `database is locked` 猝死源；<br>2. **多线程并发写极限压测验真 (`test_sqlite_concurrency_lock.py`)**: 模拟 20 个高并发线程同时向单 SQLite 数据库执行 200 次持续更新，断言 0 锁错误，单调递增版本号与状态 100% 持久化成功；<br>3. **单元测试与回归测试双全全绿**: `test_sqlite_concurrency_lock.py` (0.41s)、`test_active_notes_history.py` (5/5)、`test_agent_loop.py` (6/6) 共计 12 项测试 100% 通过；<br>4. **安全审计零泄密**: 扫描 4,424 文件零敏感信息泄露；版本号同步自增至 `1.5.58`。<br>**Commit Hash**：`1937e0962` | **修改文件**：`openviking/telemetry/telemetry_store.py`, `openviking/service/active_notes_history.py`, `openviking/observability/usage_audit/sqlite_store.py`, `openviking/ingest/cursor_store.py`, `openviking/server/oauth/storage.py`, `tests/unit/test_sqlite_concurrency_lock.py`, `package.json`, `openviking/_version.py`, `REFACTORING_PLAN.md` | [x] 已验收通过 ✅ |

...
**工单 ID**：`TASK-HARNESS-01`  
**优先级**：P0 (高优先级)  
**来源**：用户明确认可 ("100% 全无人值守自动提炼门禁 + 技能中心精简建议 + Harness 意图碰撞模拟器")  
**预计规模**：S (1 个迭代)

#### 1. 目标与描述
1. **技能中心 (`src/routes/skills/route.tsx`)**：完善 `⚡ Harness 技能资产精简与合并建议 (Low Reuse Optimization)` 交互板块，针对 166 项沉寂技能，基于物理分析给出重叠度 > 75% 的离散技能清单与自动提炼/归档列表。
2. **Harness 审计页 (`src/routes/skills/harness-logs.tsx`)**：新增 `🎯 技能意图碰撞与歧义探测模拟器`，允许在线输入自然语言，模拟测试会误触发哪个技能并给出消歧建议。
3. **后台全无人值守门禁 (Zero-Human Gate)**：搭建 AST 语法门禁 + 触发用例测试门禁，100% 自动测试提炼结果。验证通过静默落盘，验证失败静默回滚，彻底切除人工看 Diff 审核的伪自动化环节。
4. **轻量化**：完全复用现有系统 API 与轻量 `llmlingua` 工具，**绝对不下载/安装任何额外重型模型**。

#### 2. 量化验收标准
- [ ] `src/routes/skills/route.tsx` 新增重叠度 >75% 的离散技能提炼/归档列表卡片。
- [ ] `src/routes/skills/harness-logs.tsx` 新增意图碰撞与歧义探测模拟器组件。
- [ ] 具备 AST 结构门禁与自动回滚逻辑，无须人类人工审核 Diff。

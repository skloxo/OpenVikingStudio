            ...
            <div>
              <div className="flex items-center gap-1.5">
                <h3 className="text-xs font-semibold tracking-tight text-foreground">
                  {t('analyticsCharts.slaTitle', {
                    defaultValue: '核心 SLA 历史请求成功率走势',
                  })}
                </h3>
                <Tooltip>
                  <TooltipTrigger
                  ...
                  <TooltipContent side="top" className="max-w-xs text-xs">
                    {t('analyticsCharts.slaTooltip', {
                      defaultValue:
                        '展示系统全局 API 接口历史成功率走势。持续保持 99.9% 以上意味着高可靠与高可用。',
                    })}
                  </TooltipContent>
                </Tooltip>
              </div>
              <p className="text-[11px] text-muted-foreground/70 font-mono">
                {t('analyticsCharts.slaSubtitle', {
                  defaultValue: '近 7 天全量 API 接口响应成功率 (目标 > 99.9%)',
                })}
              </p>
            </div>
          </div>
                    ...
                    return (
                      <div className="rounded-md border border-border/80 bg-card p-2 shadow-none font-mono text-xs space-y-1">
                        ...
                        <div className="font-bold text-cyan-600 dark:text-cyan-400">
                          SLA 成功率: {Number(d.successRate).toFixed(2)}%
                        </div>
                        <div className="text-muted-foreground text-[11px]">
                          请求量: {d.totalRequests.toLocaleString()} 次
                        </div>
                      </div>
                    )
                  }

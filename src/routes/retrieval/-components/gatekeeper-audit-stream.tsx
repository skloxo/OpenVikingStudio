...
export function GatekeeperAuditStream() {
  ...
  const getActionBadge = (action: string) => {
    switch (action) {
      case 'noop':
        return (
          <Badge variant="outline" className="border-border/60 bg-muted/40 text-muted-foreground text-xs font-mono px-1.5 py-0">
            {t('gatekeeper.noop')}
          </Badge>
        )
      case 'update':
        return (
          <Badge variant="outline" className="border-amber-500/40 bg-amber-500/10 text-amber-600 dark:text-amber-400 text-xs font-mono px-1.5 py-0">
            {t('gatekeeper.update')}
          </Badge>
        )
      case 'delete':
        return (
          <Badge variant="outline" className="border-rose-500/40 bg-rose-500/10 text-rose-600 dark:text-rose-400 text-xs font-mono px-1.5 py-0">
            {t('gatekeeper.delete')}
          </Badge>
        )
      default:
        return (
          <Badge variant="outline" className="border-primary/40 bg-primary/10 text-primary text-xs font-mono px-1.5 py-0">
            {t('gatekeeper.add')}
          </Badge>
        )
    }
    ...
    <>
      <Card className="flex flex-col gap-2.5 p-3.5 shadow-none border-border/70">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 bg-muted/20 p-1.5 rounded-md border border-border/40">
          ...
          <div className="relative flex items-center min-w-50 max-w-xs">
            ...
            <Input
              ...
              className="h-6 pl-6 text-xs font-mono bg-background/80"
            />
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="py-6 text-center text-xs text-muted-foreground rounded-md border border-dashed border-border/50">
          ...
          <div className="flex flex-col overflow-hidden rounded-md border border-border/50 bg-background">
            {/* 表头 */}
            <div className="flex items-center gap-3 border-b border-border/50 bg-muted/20 px-3 py-1.5 text-xs font-medium text-muted-foreground">
                ...
                return (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="w-24 shrink-0 flex items-center gap-1 font-mono text-xs text-muted-foreground hover:text-foreground group cursor-pointer"
                    ...
                    {/* 目标 URI */}
                    <span
                      className="w-48 shrink-0 truncate font-mono text-xs text-foreground/90"
                      title={targetUri}
                    >
                      {targetUri}
                    </span>
                    ...
                    <span
                      className={cn(
                        'w-16 shrink-0 font-mono text-xs tabular-nums font-semibold',
                        rec.similarity >= 0.95
                          ? 'text-primary'
                          : rec.similarity >= 0.9
                    ...
                    <div className="w-12 shrink-0 text-right">
                      ...
                      >
                        ...
                        <span className="sr-only sm:not-sr-only sm:inline-block sm:ml-1 text-xs">

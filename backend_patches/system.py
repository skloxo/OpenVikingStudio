...
# 进程级物理真实 Telemetry 内存计数单例（用于 HTTP API 端点埋点）
_GLOBAL_HARNESS_METRICS = {
    "total_calls": 0,
    "find_calls": 0,
    "store_calls": 0,
    "blocked_calls": 0,
    "lessons_count": 0,
    "auto_wakeup_rate": None,
    "most_evolved_skill": None,
    "actor_peers": {},
}

def record_harness_telemetry_event(event_type: str, peer_id: str = "antigravity"):
    _GLOBAL_HARNESS_METRICS["total_calls"] += 1
    if event_type == "find":
        _GLOBAL_HARNESS_METRICS["find_calls"] += 1
    elif event_type == "store":
        _GLOBAL_HARNESS_METRICS["store_calls"] += 1
    elif event_type == "blocked":
        _GLOBAL_HARNESS_METRICS["blocked_calls"] += 1
    if peer_id:
        peers = _GLOBAL_HARNESS_METRICS["actor_peers"]
        peers[peer_id] = peers.get(peer_id, 0) + 1


@router.get("/api/v1/system/harness_metrics", tags=["system"])
async def get_harness_metrics():
    """Return real OpenViking Harness metrics from harness persistent storage.
    Falls back to in-process memory counters if harness storage is unavailable.
    Computes derived fields: auto_wakeup_rate, active_skills_count.
    """
    # 优先读取 Harness 自身持久化存储（MCP 层的真实数据）
    try:
        import os, json
        harness_data_path = os.path.expanduser("~/.openviking/harness_metrics.json")
        if os.path.exists(harness_data_path):
            with open(harness_data_path, "r") as f:
                stored = json.load(f)
            # 合并：HTTP API 端点增量叠加到 harness 存储的真实数据上
            merged = dict(stored)
            merged["find_calls"] = merged.get("find_calls", 0) + _GLOBAL_HARNESS_METRICS["find_calls"]
            merged["store_calls"] = merged.get("store_calls", 0) + _GLOBAL_HARNESS_METRICS["store_calls"]
            merged["blocked_calls"] = merged.get("blocked_calls", 0) + _GLOBAL_HARNESS_METRICS["blocked_calls"]
            merged["total_calls"] = (
                merged.get("total_calls", 0)
                + _GLOBAL_HARNESS_METRICS["find_calls"]
                + _GLOBAL_HARNESS_METRICS["store_calls"]
            )
            # 合并 actor_peers
            for peer, cnt in _GLOBAL_HARNESS_METRICS["actor_peers"].items():
                peers = merged.setdefault("actor_peers", {})
                peers[peer] = peers.get(peer, 0) + cnt

            # ── 派生字段 1：隐式自动唤醒率 (auto_wakeup_rate) ──
            # 定义：find_calls（技能检索/隐式唤醒调用）占总调用的比率
            total = merged.get("total_calls", 0)
            find = merged.get("find_calls", 0)
            if total > 0:
                merged["auto_wakeup_rate"] = round((find / total) * 100, 1)
            else:
                merged["auto_wakeup_rate"] = 0.0

            # ── 派生字段 2：技能资产活跃复用率 (active_skills_count) ──
            # 定义：被 FS Watcher 感知到最近有 IN_CLOSE_WRITE（迭代）事件的技能数量
            # 反映"有多少技能在最近被 Agent 主动写入/演进"
            try:
                from openviking.server.routers.skills import get_skill_activity_stats
                stats = get_skill_activity_stats()
                # 已迭代技能数（有写入事件的）
                iterated = stats.get("total_fs_iterations", 0)
                invoked = stats.get("total_fs_invocations", 0)
                active_count = len(stats.get("fs_iteration_counts", {})) + len(stats.get("fs_invoke_counts", {}))
                if active_count > 0:
                    merged["active_skills_count"] = active_count
                elif lessons := merged.get("lessons_count", 0):
                    # 兜底：至少 1 个技能被演进过（有 lesson 就代表至少 most_evolved_skill 活跃）
                    merged["active_skills_count"] = 1
                else:
                    merged.pop("active_skills_count", None)
            except Exception:
                merged.pop("active_skills_count", None)
        ...
        pass
    # fallback: 返回进程内存单例
    return _GLOBAL_HARNESS_METRICS

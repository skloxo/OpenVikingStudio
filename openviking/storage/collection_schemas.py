            ...
            with telemetry_ctx:
                ...
                if self._embedder:
                    from openviking.utils.embedding_input import (
                        DEFAULT_EMBEDDING_CHUNK_TOKENS,
                    ...
                    if isinstance(embedding_msg.message, str):
                        chunks = split_embedding_chunks(
                            embedding_msg.message,
                            max_chunk_tokens=DEFAULT_EMBEDDING_CHUNK_TOKENS,
                            overlap_tokens=DEFAULT_EMBEDDING_OVERLAP_TOKENS,
                        )
                    ...
                    seed_uri = self._seed_uri_for_id(uri, inserted_data.get("level", 2)) if uri else ""
                    ...
                    if len(chunks) > 1:
                        ...
                        logger.info(
                            f"[CollectionSchemas] Auto-slicing oversized text ({len(embedding_msg.message)} chars) "
                            f"into {len(chunks)} chunks for uri={uri}"
                        )
                        chunk_embed_success = True
                        for chunk_idx, chunk_text in enumerate(chunks):
                            try:
                                import time as _time

                                _embed_t0 = _time.monotonic()
                                chunk_res: EmbedResult = await embed_compat(
                                    self._embedder, chunk_text, is_query=False
                                )
                                ...
                                chunk_data["chunk_count"] = len(chunks)
                                if chunk_res.dense_vector:
                                    chunk_data["vector"] = chunk_res.dense_vector
                                if chunk_res.sparse_vector:
                                    chunk_data["sparse_vector"] = chunk_res.sparse_vector

                                if uri:
                                    id_seed = f"{account_id}:{seed_uri}:chunk_{chunk_idx}"
                                ...
                                logger.error(
                                    f"[CollectionSchemas] Failed to embed/upsert chunk {chunk_idx}/{len(chunks)} for {uri}: {chunk_err}"
                                )
                                chunk_embed_success = False
                                break

                        if chunk_embed_success:
                            self._merge_request_stats(embedding_msg.telemetry_id, processed=1)
                            self._record_request_success(embedding_msg)
                            report_success = True
                    ...
                    # Single-message embedding flow (message is list or len(chunks) <= 1)
                    try:
                        import time as _time
                        ...
                        if error_class == ERROR_CLASS_INPUT_TOO_LARGE:
                            logger.warning(
                                "[CollectionSchemas] Input too large for embedder context, generating fallback vector with truncated head (1500 chars): %s",
                                error_msg,
                            )
                            try:
                                if isinstance(embedding_msg.message, str):
                                    truncated_text = embedding_msg.message[:1500]
                                    result = await embed_compat(
                                        self._embedder, truncated_text, is_query=False
                                    )
                                else:
                                    result = None
                            except Exception as truncate_err:
                                logger.error("Failed fallback embedding for truncated text: %s", truncate_err)
                                result = None
                        ...
                        elif error_class == ERROR_CLASS_AUTH:
                ...
                try:
                    ...
                    if record_id:
                        logger.debug(
                            f"Successfully wrote embedding to database: {record_id} abstract {inserted_data.get('abstract')} vector {inserted_data.get('vector', [])[:5]}"
                        )

...
class Session:
    ...
    def _parse_wm_sections(text: str) -> Dict[str, str]:
        ...
        return WorkingMemorySynthesizer.parse_wm_sections(text)

    _WM_SECTION_BULLET_THRESHOLD = WorkingMemorySynthesizer._WM_SECTION_BULLET_THRESHOLD
    _WM_SECTION_TOKEN_THRESHOLD = WorkingMemorySynthesizer._WM_SECTION_TOKEN_THRESHOLD
    _WM_OVERSIZED_APPEND_CAP = WorkingMemorySynthesizer._WM_OVERSIZED_APPEND_CAP
    _WM_CONSOLIDATION_SENTINEL = WorkingMemorySynthesizer._WM_CONSOLIDATION_SENTINEL
    _WM_APPEND_ONLY_SECTIONS = WorkingMemorySynthesizer._WM_APPEND_ONLY_SECTIONS
    _WM_PATH_LIKE_RE = WorkingMemorySynthesizer._WM_PATH_LIKE_RE
    _WM_TITLE_STOPWORDS = WorkingMemorySynthesizer._WM_TITLE_STOPWORDS
    _WM_KEY_FACTS_MIN_BULLET_RATIO = WorkingMemorySynthesizer._WM_KEY_FACTS_MIN_BULLET_RATIO
    _WM_KEY_FACTS_MIN_ANCHOR_COVERAGE = WorkingMemorySynthesizer._WM_KEY_FACTS_MIN_ANCHOR_COVERAGE
    _WM_ANCHOR_DATE_RE = WorkingMemorySynthesizer._WM_ANCHOR_DATE_RE
    _WM_ANCHOR_NUMBER_RE = WorkingMemorySynthesizer._WM_ANCHOR_NUMBER_RE
    _WM_ANCHOR_DECISION_RE = WorkingMemorySynthesizer._WM_ANCHOR_DECISION_RE
    _WM_ANCHOR_STOPWORDS = WorkingMemorySynthesizer._WM_ANCHOR_STOPWORDS

    @staticmethod
    def _build_wm_section_reminders(overview: str) -> str:
        """Compute dynamic section-size warnings for the WM update prompt."""
        return WorkingMemorySynthesizer.build_wm_section_reminders(overview)
    ...
    def _wm_recover_ops_from_raw(raw_str: str) -> Dict[str, Any]:
        ...
        return WorkingMemorySynthesizer.recover_ops_from_raw(raw_str)

    @staticmethod
    def _wm_extract_bullet_items(text: str) -> List[str]:
        """Extract bullet-like items from a markdown section body."""
        return WorkingMemorySynthesizer.extract_bullet_items(text)
    ...
    def _wm_enforce_append_only(header: str, op: Any, old_content: str) -> Dict[str, Any]:
        ...
        return WorkingMemorySynthesizer.enforce_append_only(header, op, old_content)

    @staticmethod
    def _extract_lexical_anchors(text: str) -> set:
        """Extract fact-preserving anchors: dates, numbers, proper nouns, decision markers."""
        return WorkingMemorySynthesizer.extract_lexical_anchors(text)
    ...
    def _salvage_new_items_from_rejected_update(
        ...
        return WorkingMemorySynthesizer.salvage_new_items_from_rejected_update(new_content, old_content)
    ...
    def _wm_enforce_key_facts_consolidation(op: Any, old_content: str) -> Dict[str, Any]:
        ...
        return WorkingMemorySynthesizer.enforce_key_facts_consolidation(op, old_content)
    ...
    def _wm_enforce_files_no_regression(op: Any, old_content: str) -> Dict[str, Any]:
        ...
        return WorkingMemorySynthesizer.enforce_files_no_regression(op, old_content)
    ...
    def _wm_enforce_title_stability(op: Any, old_content: str) -> Dict[str, Any]:
        ...
        return WorkingMemorySynthesizer.enforce_title_stability(op, old_content)
    ...
    def _wm_enforce_open_issues_resolved(op: Any, old_content: str) -> Dict[str, Any]:
        ...
        return WorkingMemorySynthesizer.enforce_open_issues_resolved(op, old_content)

    @staticmethod
    def _merge_wm_sections(old_wm: str, ops: Dict[str, Any]) -> str:
        """Merge LLM per-section ops into a new Working Memory document."""
        return WorkingMemorySynthesizer.merge_wm_sections(old_wm, ops)

...
## 📌 v1.2.34：技能中心全托管感应引擎 + Harness 真实数据打通
- **交付时间**：2026-08-04
- **Git Commit**：`cefba2b`  **Git Tag**：`v1.2.34`
- **用户验收**：待验收 ⏳
- **修改文件**：
  - [`skills.py`](file:///home/skloxo/.local/lib/python3.12/site-packages/openviking/server/routers/skills.py)（OpenViking 后端源码）
  - [`system.py`](file:///home/skloxo/.local/lib/python3.12/site-packages/openviking/server/routers/system.py)（OpenViking 后端源码）
  - [`openviking-studio-dev/SKILL.md`](file:///home/skloxo/.gemini/config/skills/openviking-studio-dev/SKILL.md)
- **交付内容**：
  1. **技能中心全托管感应引擎**（`skills.py`）：
     - Linux `inotify`（via `ctypes/libc`）OS 级感知，`IN_ACCESS` → 调用事件，`IN_CLOSE_WRITE` → 迭代事件
     - 无论 IDE 直接读磁盘还是 API 调用，VK 全量感知；优先 inotify，降级 atime/mtime 轮询
     - 覆盖三个托管目录：`~/.openviking/skills`、`~/.gemini/config/skills`、`.agents/skills`
  2. **Harness Metrics 真实数据打通**（`system.py`）：
     - `get_harness_metrics()` 读取 `~/.openviking/harness_metrics.json` MCP 持久化真实数据
     - 修复路径 bug：`~/.openviking/data/` → `~/.openviking/`
     - 合并 HTTP 增量与 MCP 历史；实测 `inotify_invoke: 542`
  3. **SKILL.md 真实 Reflexion**：删除 17 条伪造模板，写入 L1–L7 真实踩坑铁律

...
### 14. 🛡️ 知识库健康度多维雷达全量真实巡检与异步工单化 (Card-Hygiene-AsyncAuditTaskCenter / v1.5.56)

- **直达链接**：[http://127.0.0.1:1933/studio/home](http://127.0.0.1:1933/studio/home) ｜ 任务中心：[http://127.0.0.1:1933/studio/tasks](http://127.0.0.1:1933/studio/tasks)
- **验证目的**：验证知识卫生雷达卡片切除 `LIMIT 200` 假静态抽样、展示真实在籍全量巡检篇数（2,036+ 篇）、一键异步发起全量巡检任务并在任务中心沉淀可追溯的第一公民工单。
- **30秒操作步骤与看点**：
  1. **访问首页并定位雷达卡片**：
     - 打开浏览器访问 `http://127.0.0.1:1933/studio/home`；
     - 向下滚动定位到「知识库健康度多维雷达」卡片。
  2. **核验卡片真实在籍全量数据**：
     - 右侧核心统计显式回显「在籍全量巡检: 2,036 篇」（或最新在籍总数，绝对不再是写死的 200 篇）；
     - 底部状态显示湛蓝中性冰青盾牌图标 `ShieldCheckIcon`，回显「全量健康良好」与相对时间戳（如“刚刚”或“X秒前”）；
     - 底部带有 `#工单` 超链接，可直达关联任务。
  3. **点击「立即全量巡检」验证异步触发**：
     - 点击右上角「立即全量巡检」按钮，观察刷新图标旋转动画；
     - 耗时毫秒级刷新（约 60ms），卡片底部更新最新耗时与工单号。
  4. **前往任务中心核验第一公民工单**：
     - 访问 `http://127.0.0.1:1933/studio/tasks`；
     - 列表顶部出现最新「知识卫生全量巡检」任务，状态为「已完成 (100%)」；
     - 点击展开工单抽屉，核验 3 大工序进度（全库记忆扫描 100%、多维卫生诊断 100%、巡检报告发布 100%）与交付物卡片（如 `全库已巡检 2,036 篇记忆 · 综合健康度 100 分 · 冲突 0 项 · 休眠 0 项 · 孤立 0 项`）。
  5. **负向红线排查**：全卡片与任务详情绝无任何绿色 (NO GREEN EVER 🚫)，最小字号严格 $\ge 12\text{px}$，进度严格遵循完成才 $+1$ 的 $X/Y$ 物理契约。
- **合格标准**：不再显示 200 假抽样，任务中心实时生成工单并完整呈现 3 阶交付物。
